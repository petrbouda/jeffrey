...
public class AdvisorConfiguration {
    ...
    /**
     * Stores a finished batch's timeline in the profile database. Built here rather than injected so the
     * runner stays free of persistence types; opens the profile DB per call, like the other factories.
     */
    private AdvisorRunResultWriter runResultWriter() {
        return (profile, result) -> {
            DataSource profileDb = databaseManagerResolver.open(profile);
            profileRepositories.newAdvisorRepository(profileDb)
                    .storeRunResult(new AdvisorRunResultRow(Json.toString(result), result.completedAt()));
        };
    }
