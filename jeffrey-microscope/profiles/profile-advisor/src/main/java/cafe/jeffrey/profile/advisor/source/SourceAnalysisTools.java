...
/**
 * Read-only filesystem tools the AI uses to navigate a {@link SourceTree} the way an agentic code
 * assistant would: list directories, glob for files, read a file, and grep file contents. Every path
 * argument is resolved against {@link #root} and rejected if it escapes it, so the model can never read
 * outside the configured source folder. There are deliberately no write, delete or execute tools — an
 * advisor run only ever reads source.
 *
 * <p>The containment check matters more here than it did against a throwaway clone: the root is a
 * directory the user works in, sitting inside a home directory full of things the model has no business
 * reading. So containment is enforced twice — once on the lexically normalized path, and again on the
 * real path, which is what catches a symlink inside the tree pointing out of it.</p>
 *
 ...
 */
public class SourceAnalysisTools {

    private static final Logger LOG = LoggerFactory.getLogger(SourceAnalysisTools.class);
    ...
    public SourceAnalysisTools(Path root) {
    ...
    public String glob(
        ...
        try (Stream<Path> walk = Files.walk(root, MAX_WALK_DEPTH)) {
            walk.filter(Files::isRegularFile)
                    ...
                    .filter(path -> !escapesViaSymlink(path))
                    .map(root::relativize)
    ...
    /**
     * Resolves a repository-relative path and returns it only if it stays within the source tree and
     * exists; otherwise {@code null}. Blocks {@code ../} traversal, absolute paths, and symlinks that
    ...
    private Path resolveSafe(String relative) {
        ...
        return escapesViaSymlink(resolved) ? null : resolved;
    }

    /**
     * True when the path's real location falls outside the root. The root itself is already a real path
     * (the resolver canonicalizes it), so the comparison is between two symlink-free paths. An
     * unreadable path is treated as an escape: refusing a file we cannot classify is the safe default.
     */
    private boolean escapesViaSymlink(Path resolved) {
        try {
            return !resolved.toRealPath().startsWith(root);
        } catch (IOException e) {
            LOG.warn("Failed to resolve real path, treating as outside the source tree: message={}",
                    e.getMessage());
            return true;
        }
    }
