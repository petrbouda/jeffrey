 ...
 * the three mean different things to whoever reads them: one was attached by hand, one is the event
 * type's own schema, and one is a column every span already had.
 * <p>
 * The names are stored in {@code trace_span_attributes.source} and read back out of profiles that
 * outlive the code that wrote them, so they are frozen: never renamed, only added to.
 */
