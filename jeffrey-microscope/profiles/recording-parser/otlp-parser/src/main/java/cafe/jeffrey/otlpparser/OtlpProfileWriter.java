 ...
 * per-sample timestamps). OTLP allows a single {@code sample_type} per {@link Profile}, so the export
 * carries exactly one dimension. The result is a single serialized (uncompressed) raw
 * {@code ProfilesData}, which {@link OtlpStreamReader} reads back directly.
 */
public final class OtlpProfileWriter {
    ...
    /**
     * Builds the OTLP profiles message for the given dimension and samples.
     *
    ...
    public byte[] write(
        ...
        Profile.Builder profile = Profile.newBuilder()
                .setSampleType(ValueType.newBuilder()
                        .setTypeStrindex(string(valueType.type()))
                        .setUnitStrindex(string(valueType.unit())))
