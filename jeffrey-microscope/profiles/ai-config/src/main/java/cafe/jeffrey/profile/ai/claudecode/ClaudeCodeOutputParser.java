...
final class ClaudeCodeOutputParser {
    ...
    ClaudeCodeOutputParser(ObjectMapper objectMapper) {
    ...
    ClaudeCodeResult parse(List<String> lines) {
        ...
        for (String line : lines) {
            ...
            String type = node.path(EVENT_TYPE).asText("");
            switch (type) {
                ...
                case EVENT_TYPE_RESULT -> {
                    resultText = node.path(EVENT_TYPE_RESULT).asText("");
                    error = node.path("is_error").asBoolean(false)
                            || !RESULT_SUBTYPE_SUCCESS.equals(node.path("subtype").asText(RESULT_SUBTYPE_SUCCESS));
                }
                default -> {
    ...
    private static void collectAssistantToolUses(JsonNode assistantEvent, Set<String> toolsUsed) {
        ...
        for (JsonNode block : content) {
            if (CONTENT_TYPE_TOOL_USE.equals(block.path(EVENT_TYPE).asText(""))) {
                addToolName(block.path("name").asText(""), toolsUsed);
            }
        }
    }
    ...
    private static void collectStreamEventToolUse(JsonNode streamEvent, Set<String> toolsUsed) {
        ...
        if (CONTENT_TYPE_TOOL_USE.equals(contentBlock.path(EVENT_TYPE).asText(""))) {
            addToolName(contentBlock.path("name").asText(""), toolsUsed);
        }
    }
