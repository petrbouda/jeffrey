...
-- `scope_id` is '' rather than NULL for pipelines that run once per profile (the heap dump), and names
-- the sub-unit for a pipeline that runs several times over one profile — so the primary key works
