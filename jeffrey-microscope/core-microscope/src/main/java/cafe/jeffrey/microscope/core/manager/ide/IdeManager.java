...
/**
 * Forwards "open in IDE" requests from the frontend to a locally-running IDE plugin (e.g. the
 * IntelliJ plugin listening on {@code http://localhost:4243}). The browser talks only to the
 * same-origin Microscope backend; this class performs the cross-port hop server-side, which avoids
 * the CORS / mixed-content problems of calling the plugin directly from the SPA.
 *
 * <p>Reaching the IDE is best-effort: when the plugin is disabled, unconfigured, unreachable, or
 * rejects the request, {@link #open(IdeOpenRequest)} returns a failed {@link IdeOpenResult} with a
 * human-readable message rather than throwing — the IDE being offline is an expected condition.
 */
public class IdeManager {

    private static final Logger LOG = LoggerFactory.getLogger(IdeManager.class);
    ...
    public IdeManager(boolean enabled, String baseUrl) {
    ...
    public IdeOpenResult open(IdeOpenRequest request) {
        ...
        if (baseUrl == null) {
            LOG.debug("IDE open requested but base URL is not configured");
        ...
        try {
            ...
            if (status.is2xxSuccessful()) {
                LOG.debug("IDE open dispatched: url={} status={}", url, status.value());
            ...
            LOG.debug("Failed to reach IDE plugin: url={} reason={}", url, e.getMessage());
