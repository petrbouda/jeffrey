    ...
    public record GuardResponse(
        ...
        static GuardResponse from(GuardianGuard guard) {
            ...
            return new GuardResponse(
                    guard.guardId(), guard.name(), guard.enabled(), guard.builtIn(), guard.eventType(),
                    guard.category(), guard.resultType(), guard.targetFrame(), guard.matchingType(),
                    guard.infoThreshold(), guard.warningThreshold(), guard.minSamples(), guard.matcherSpec(),
                    guard.preconditions(), guard.summaryNoun(), guard.explanation(), guard.solution(),
