...
class SettingsManagerTest {
    ...
    class UpsertSetting {
        ...
        void storesPlainValueForNonSecret() {
            manager.upsert("ai", "jeffrey.microscope.ai.provider", "claude", false);

            verify(settingsRepository).upsert(new Setting("ai", "jeffrey.microscope.ai.provider", "claude", false));
        ...
        void encryptsValueForSecret() {
            ...
            manager.upsert("ai", "jeffrey.microscope.ai.api-key", "sk-ant-api03-key", true);

            verify(secretEncryptor).encrypt("sk-ant-api03-key");
            verify(settingsRepository).upsert(new Setting("ai", "jeffrey.microscope.ai.api-key", "encrypted-base64", true));
        }
    }
