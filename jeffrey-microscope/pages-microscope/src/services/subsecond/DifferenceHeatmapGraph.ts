...
export default class DifferenceHeatmapGraph {
  ...
  #setupStrokeWidth() {
    const rect: Element | null = document.querySelector(
      this.elementQueryId + ' rect[i="' + (this.sizeY - 1) + '"][j="0"]'
    );
    if (rect) {
  ...
  #setupTooltipPositionAndStyle() {
    ...
    if (el) {
      el.setAttribute(
        'style',
        'background: transparent; padding: 10px; border: none; box-shadow: none;'
      );
    } else {
      setTimeout(() => {
  ...
  #options(seriesData: SubSecondSerie[]) {
    const numColumns =
      seriesData.length > 0 && seriesData[0].data.length > 0 ? seriesData[0].data.length : 300;
        ...
        heatmap: {
          ...
          colorScale: {
            ranges: [
              {
                from: -Infinity,
                to: -thresholds.veryLarge,
                color: '#155724',
                name: 'Very large decrease'
              },
              {
                from: -thresholds.veryLarge,
                to: -thresholds.large,
                color: '#28a745',
                name: 'Large decrease'
              },
              {
                from: -thresholds.large,
  ...
  #calculateThresholds(absMax: number): {
    small: number;
    medium: number;
    large: number;
    veryLarge: number;
  ...
  #_calculateHighlightedArea(x1: number, y1: number, x2: number, y2: number): HTMLDivElement[] {
    ...
    rects.push(
      this.#createHighlightElement(
        fRectLeft - scrollLeft,
        fRectTop + strokeWidth,
        fRectHeight,
        fRectWidth
      )
    );
    ...
    rects.push(
      this.#createHighlightElement(
        lRectLeft - scrollLeft,
        lRectTop + strokeWidth,
        lRectHeight,
        lRectWidth
      )
    );
    ...
    if (x2 - x1 > 1) {
      ...
      rects.push(
        this.#createHighlightElement(
          mRectLeft - scrollLeft,
          mRectTop + strokeWidth,
          mRectHeight,
          mRectWidth
        )
      );
