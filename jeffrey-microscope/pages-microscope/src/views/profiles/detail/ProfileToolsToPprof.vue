...
<template>
  <PageHeader
    ...
    description="Export a single stack-based event type as a standard, valid pprof (.pb.gz) file — download it or add it back to Recordings."
  >
      ...
      <div class="col-lg-6">
        ...
        >
          ...
          <span class="weight-option__body">
            <span class="weight-option__title">Include weight as a second value type</span>
            <span class="weight-option__hint">
              <template v-if="selectedEvent && selectedEvent.hasWeight">
                Adds the event's weight ({{ selectedEvent.category }}) alongside sample count.
              </template>
        ...
        <div class="summary-card">
          ...
          <template v-if="selectedEvent">
            ...
            <div class="summary-kv">
              <span>pprof sample_type</span>
              <span class="mono">{{ sampleTypeText }}</span>
            </div>
          </template>
          <div v-else class="text-muted small py-3">
            Select an event type to see its details and pprof mapping.
          </div>
        </div>
...
const eventTypes = ref<PprofExportEventType[]>([]);
...
const selectedEvent = computed<PprofExportEventType | null>(() => {
...
const weightText = computed(() => {
  ...
  // weightSampleType is `type/unit` (e.g. `alloc_space/bytes`); the unit drives the formatter.
...
const filename = computed(() => {
  ...
  const eventShort = selectedEvent.value
    ...
    : 'event';
  return `jeffrey-${safeProfileId}-${eventShort}.pb.gz`;
});
...
async function convertAndDownload(): Promise<void> {
  ...
  try {
    ...
    const blob = await client.downloadPprof(selectedEvent.value.code, includeWeight.value);
    ...
    ToastService.success('Downloaded', 'pprof file saved.');
  } catch (error) {
    console.error('pprof download failed:', error);
    ToastService.error('Download Failed', 'Could not generate the pprof file.');
  } finally {
...
async function addToRecordings(): Promise<void> {
  ...
  try {
    ...
    ToastService.error('Failed', 'Could not add the pprof file to Recordings.');
  } finally {
    adding.value = false;
  }
}
...
onMounted(async () => {
  try {
    const client = new ProfileToolsClient(profileId);
    eventTypes.value = await client.pprofEventTypes();
  } catch (error) {
    console.error('Failed to load pprof event types:', error);
