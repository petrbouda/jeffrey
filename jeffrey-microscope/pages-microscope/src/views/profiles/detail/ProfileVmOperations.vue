    ...
    <div v-else>
      ...
      <div v-show="activeTab === 'operations'">
        ...
        <DataTable v-else>
          <template #toolbar>
            <TableToolbar
              v-model="vmOperationsView.query"
              search-placeholder="Filter operations..."
            >
              <span class="toolbar-info">VM operations</span>
              <template #filters>
                <Badge
                  key-label="Total"
                  :value="vmOperationsView.matchCount"
                  variant="secondary"
                  size="s"
                  borderless
                />
          ...
          <tbody>
            <tr v-for="operation in vmOperationsView.visible" :key="operation.operation">
              ...
              <td class="text-end">
                {{ FormattingService.formatDuration2Units(operation.totalNanos) }}
              </td>
              <td class="text-end">
                {{ FormattingService.formatDuration2Units(operation.maxNanos) }}
              </td>
              <td>
      ...
      <div v-show="activeTab === 'safepoints'">
        ...
        <div class="chart-container mb-4">
          <TimeSeriesChart
            :primary-data="pauseSeries"
            primary-title="Safepoint Pause Time"
        ...
        >
          Measuring how long the JVM waits for all threads to <em>reach</em> a safepoint (TTSP)
          needs <code>jdk.SafepointStateSynchronization</code> events, which are disabled in the
          default and profile JFR configurations. The pause timeline above (the time spent
          <em>at</em> safepoints) is always available.
        </DisabledEventsNotice>
        <template v-else>
          <ChartDescription
          ...
          <div class="chart-container">
            <TimeSeriesChart
              :primary-data="safepointSeries"
              primary-title="Time to Safepoint"
              :primary-axis-type="AxisFormatType.DURATION_IN_NANOS"
              :visible-minutes="60"
            />
          </div>
      ...
      <div v-show="activeTab === 'about'">
        ...
        >
          <AboutCallout variant="intro">
            <p>
              Many JVM tasks need every Java thread paused at a known-good state. The JVM achieves
              this with a <strong>safepoint</strong>: it asks all threads to stop at the next poll
              point, waits for the slowest one (that wait is <em>time-to-safepoint</em>), runs the
              <strong>VM operation</strong>, then releases everyone. GC is the most common reason,
              but far from the only one — and none of it shows up in a CPU profile.
            </p>
          </AboutCallout>
          ...
          <AboutSection icon="bi-stopwatch" title="The Safepoint Protocol">
            <FeatureGrid>
              <FeatureCard icon="bi-sign-stop" variant="danger" title="What a safepoint is">
                A point where a thread's state (stack, registers, references) is consistent for the
                JVM to inspect or modify. Compiled code has <em>poll points</em> (loop back-edges,
                method returns) where a thread checks the safepoint flag and parks.
              </FeatureCard>
              <FeatureCard
                icon="bi-hourglass-bottom"
                variant="warning"
                title="Time to Safepoint (TTSP)"
              >
                The operation can't start until the <em>last</em> thread reaches a poll point. One
                thread in a long counted loop, a JNI critical section, or page-faulting can stall
                every other thread — TTSP is often more surprising than the operation's own
                duration.
              </FeatureCard>
              ...
              <FeatureCard
                ...
                variant="success"
                title="Why it's usually fine"
              >
                Safepoints are cheap and frequent. They become a problem only when TTSP is high (a
                slow-to-poll thread) or an operation is unexpectedly long (a heap inspection, a
                class redefinition storm).
              </FeatureCard>
            </FeatureGrid>
          </AboutSection>
          ...
          <AboutSection icon="bi-list-task" title="Types of VM Operation">
            <FeatureGrid>
              ...
              <FeatureCard icon="bi-unlock" variant="info" title="Bias revocation">
                <code>RevokeBias</code> / <code>BulkRevokeBias</code> — undoing biased locking when
                a second thread contends an object. Common in older JVMs; removed in JDK 15+.
              </FeatureCard>
              ...
              <FeatureCard icon="bi-arrow-repeat" variant="danger" title="Class redefinition">
                <code>RedefineClasses</code> / <code>RetransformClasses</code> — instrumentation
                agents rewriting bytecode at runtime. Heavy use means recurring pauses plus
                metaspace churn.
              </FeatureCard>
          ...
          <AboutSection icon="bi-broadcast" title="How JFR Emits This">
            <ul>
              <li>
                ...
                <code>safepoint</code> / <code>blocking</code> flags and duration. Enabled by
                default; drives the operations table and the pause timeline.
              </li>
              <li>
                <code>jdk.SafepointStateSynchronization</code> (with the
                <code>SafepointBegin</code> / <code>SafepointEnd</code> family) — time-to-safepoint
                and the safepoint phases. <strong>Off by default</strong>, so the TTSP chart is
                usually gated until enabled.
              </li>
            </ul>
          </AboutSection>
        </AboutPanel>
...
const safepointSeries = computed<number[][]>(
  () => safepointTimeline.value?.series?.[0]?.data ?? []
);
  ...
  return [
    {
      ...
      breakdown: [
        { label: 'VM Operations', value: FormattingService.formatNumber(o.vmOperationCount) }
      ]
    },
    {
      ...
      breakdown: o.longestPauseOperation
        ? [{ label: 'Operation', value: o.longestPauseOperation }]
        : []
    },
    {
      icon: 'list-task',
