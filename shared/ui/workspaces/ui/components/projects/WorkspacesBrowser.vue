...
<template>
  <div class="projects-layout">
    ...
    <!-- Drawers -->
    <AddHubModal v-model:show="showAddHubModal" @server-added="handleServerAdded" />
...
import WorkspaceClient from '@workspaces/services/api/WorkspaceClient';
import WorkspaceProjectsClient from '@workspaces/services/api/WorkspaceProjectsClient';
import ProjectClient from '@workspaces/services/api/ProjectClient';
import RemoteServer from '@workspaces/services/api/model/RemoteServer';
import Workspace from '@workspaces/services/api/model/Workspace';
import Project from '@workspaces/services/api/model/Project';
...
const probeServer = async (hubId: string) => {
  try {
    await new WorkspaceClient(hubId).list({ suppressToast: true });
    serverStatuses.value = { ...serverStatuses.value, [hubId]: 'online' };
  } catch {
    serverStatuses.value = { ...serverStatuses.value, [hubId]: 'offline' };
  }
};
...
const refreshServers = async () => {
  try {
    ...
    for (const s of servers.value) {
      initial[s.id] = serverStatuses.value[s.id] ?? 'unknown';
    }
...
const selectServer = (hubId: string) => {
  if (selectedServerId.value === hubId) {
    return;
  }
  selectedServerId.value = hubId;
  selectedWorkspaceId.value = null;
  projects.value = [];
  closeCreateForm();
  refreshWorkspaces();
...
const submitCreate = async () => {
  if (!selectedServerId.value || !isCreateFormValid.value) {
    return;
  }
  creating.value = true;
  createError.value = null;
