...
import cafe.jeffrey.shared.ui.hub.dto.HubResponse;
import cafe.jeffrey.shared.ui.hub.request.AddHubRequest;

import java.util.List;

/**
 * Shared WorkspaceBrowser controller for the connected jeffrey-hub registry. Both deployments
 * register it via {@code HubsFeatureConfiguration} and supply a {@link HubRegistry} bridge over
...
public class HubsController {
    ...
    @PostMapping
    public ResponseEntity<HubResponse> add(@RequestBody AddHubRequest request) {
        validate(request);
        ...
        // for an unreachable / misconfigured hub instead of a stored unusable pointer.
        try {
    ...
    private static void validate(AddHubRequest request) {
        if (request.name() == null || request.name().isBlank()) {
            throw Exceptions.invalidRequest("Hub name is required");
        }
