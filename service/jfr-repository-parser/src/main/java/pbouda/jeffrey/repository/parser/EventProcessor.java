...
/**
 * Processor for JFR events from a repository. Implements the event processing logic
 * and provides the accumulated result.
 *
...
public interface EventProcessor<T> extends Supplier<T> {
    ...
    /**
     * A collection contains all names of events that can be processed by the implementation of this interface.
    ...
    /**
     * Called when metadata is available from the event stream.
     *
     * @param eventTypes list of available event types
     */
    default void onMetadata(List<EventType> eventTypes) {
    }

    /**
     * Processes incoming event. This method is invoked after calling {@link #onStart()},
