...
public class IntervalParser {
    ...
    public static final Duration DEFAULT_INTERVAL = Duration.ofMillis(10);
    ...
    public static Duration parse(String interval) {
        ...
        if (interval.endsWith("ms")) {
            return Duration.ofMillis(Long.parseLong(interval.substring(0, interval.length() - 2)));
        } else if (interval.endsWith("s")) {
            return Duration.ofSeconds(Long.parseLong(interval.substring(0, interval.length() - 1)));
        } else if (interval.endsWith("us")) {
            long micros = Long.parseLong(interval.substring(0, interval.length() - 1));
