...
public abstract class DuckDBTimeseriesQueries {
    ...
    public static final String SIMPLE_TIMESERIES_QUERY = """
            ...
            WHERE (events.profile_id = :profile_id
                ...
                AND (:included_tags IS NULL OR stacktraces.tag_ids IS NULL OR array_length(stacktraces.tag_ids) = 0 OR list_has_any(stacktraces.tag_ids, [:included_tags]))
                AND (:excluded_tags IS NULL OR stacktraces.tag_ids IS NULL OR array_length(stacktraces.tag_ids) = 0 OR NOT list_has_any(stacktraces.tag_ids, [:excluded_tags]))
