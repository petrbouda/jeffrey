...
class JdbcAlertRepositoryTest {
    ...
    class InsertMethod {
        ...
        @Test
        void insertsAlert(DataSource dataSource) throws SQLException {
            var provider = new DatabaseClientProvider(dataSource);
            TestUtils.executeSql(dataSource, "sql/messages/insert-project-for-messages.sql");
            var repository = new JdbcAlertRepository("proj-001", provider);

            repository.insert(alert("HIGH_CPU_USAGE", "session-1", T1));
            ...
            assertEquals("Alert Title", msg.title());
            assertEquals("Alert text", msg.message());
            assertEquals(Severity.HIGH, msg.severity());
            assertEquals("RESOURCE", msg.category());
            assertEquals("monitor", msg.source());
        ...
        void differentType_sameSessionAndTimestamp_bothInserted(DataSource dataSource) throws SQLException {
            ...
            repository.insert(alert("HIGH_CPU_USAGE", "session-1", T1));
            repository.insert(alert("LOW_MEMORY", "session-1", T1));
        ...
        void sameType_differentTimestamp_bothInserted(DataSource dataSource) throws SQLException {
            ...
            repository.insert(alert("HIGH_CPU_USAGE", "session-1", T1));
            repository.insert(alert("HIGH_CPU_USAGE", "session-1", T2));
    ...
    class FindAllMethod {
        ...
        @Test
        void returnsAlertsInTimeRange_newestFirst(DataSource dataSource) throws SQLException {
            var provider = new DatabaseClientProvider(dataSource);
            TestUtils.executeSql(dataSource, "sql/messages/insert-project-for-messages.sql");
            var repository = new JdbcAlertRepository("proj-001", provider);

            repository.insert(alert("TYPE_A", "session-1", T1));
            repository.insert(alert("TYPE_B", "session-1", T2));
            repository.insert(alert("TYPE_C", "session-1", T3));
        ...
        @Test
        void returnsEmpty_whenNoAlertsInRange(DataSource dataSource) throws SQLException {
            var provider = new DatabaseClientProvider(dataSource);
            TestUtils.executeSql(dataSource, "sql/messages/insert-project-for-messages.sql");
            var repository = new JdbcAlertRepository("proj-001", provider);
        ...
        @Test
        void excludesAlertsOutsideRange(DataSource dataSource) throws SQLException {
            var provider = new DatabaseClientProvider(dataSource);
            TestUtils.executeSql(dataSource, "sql/messages/insert-project-for-messages.sql");
            var repository = new JdbcAlertRepository("proj-001", provider);
        ...
        void filtersByProjectId(DataSource dataSource) throws SQLException {
            ...
            var repo1 = new JdbcAlertRepository("proj-001", provider);
            var repo2 = new JdbcAlertRepository("proj-002", provider);

            repo1.insert(alert("TYPE_A", "session-1", T1));
            repo2.insert(alert("TYPE_B", "session-2", T2));
    ...
    class DeleteOlderThanMethod {
        ...
        private static long countAlerts(DataSource dataSource) {
            var jdbc = new NamedParameterJdbcTemplate(dataSource);
            Long count = jdbc.queryForObject("SELECT COUNT(*) FROM alerts", Map.of(), Long.class);
            return count != null ? count : 0;
        }

        @Test
        void deletesOldAlerts_andReturnsCount(DataSource dataSource) throws SQLException {
            var provider = new DatabaseClientProvider(dataSource);
            TestUtils.executeSql(dataSource, "sql/retention/insert-retention-test-data.sql");
            var repository = new JdbcAlertRepository("", provider);

            int deleted = repository.deleteOlderThan(CUTOFF);

            assertEquals(2, deleted);
            assertEquals(1, countAlerts(dataSource));
        }

        @Test
        void returnsZero_whenNoOldAlerts(DataSource dataSource) throws SQLException {
            var provider = new DatabaseClientProvider(dataSource);
            TestUtils.executeSql(dataSource, "sql/messages/insert-project-for-messages.sql");
            var alertRepo = new JdbcAlertRepository("proj-001", provider);
            alertRepo.insert(alert("TYPE_A", "session-1", Instant.parse("2025-06-25T10:00:00Z")));
