...
<template>
  <MainCard no-padding>
    ...
    <EmptyState
      ...
      message="When the same hotspot is confirmed in two or more projects, it will surface here — those are usually fixed once, upstream, rather than in every caller."
    />
    ...
    <div v-else class="fleet">
      <div v-for="pattern in patterns" :key="pattern.citedFrame" class="group">
        <button class="group-head" @click="toggle(pattern.citedFrame)">
          <i class="bi" :class="isOpen(pattern.citedFrame) ? 'bi-chevron-down' : 'bi-chevron-right'"></i>
          ...
          <span class="count">{{ pattern.projectCount }} / {{ analyzedProjectCount }} projects</span>
          <Badge
        ...
        <div v-if="isOpen(pattern.citedFrame)" class="occurrences">
          <button
            ...
            :key="`${occurrence.projectId}-${occurrence.recordingId}`"
            class="occurrence"
            @click="$emit('open', occurrence)"
          >
            <span class="project">{{ occurrence.projectName ?? occurrence.projectId }}</span>
...
import type { FleetOccurrence, FleetPattern } from '@/services/api/model/FleetPatterns';
