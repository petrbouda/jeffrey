...
/**
 * SQLite-backed {@link HubsRepository} for the performance-analyst store. Mirrors microscope's DuckDB
 * {@code JdbcHubsRepository} but stores {@code created_at} as epoch millis and {@code plaintext} as
 * INTEGER 0/1.
 */
public class JdbcHubsRepository implements HubsRepository {
    ...
    public HubInfo create(HubInfo serverInfo) {
        MapSqlParameterSource params = new MapSqlParameterSource()
                ...
                .addValue("plaintext", serverInfo.address().plaintext() ? 1 : 0)
                .addValue("created_at", serverInfo.createdAt().toEpochMilli());
