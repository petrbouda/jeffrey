  ...
  return [
    {
      ...
      variant: ((header.successRate || 0) >= 0.99 ? 'info' : header.errorCount > 0 ? 'danger' : 'warning') as const,
      breakdown: [
        { label: 'Success', value: `${((header.successRate || 0) * 100).toFixed(1)}%`, color: (header.successRate || 0) >= 0.99 ? '#28a745' : '#ffc107' },
