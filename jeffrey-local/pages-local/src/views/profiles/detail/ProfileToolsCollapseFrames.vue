...
<template>
  <PageHeader icon="bi-layers" title="Collapse Frames" description="Replace consecutive stacktrace frames matching class name patterns with a single synthetic frame. Simplifies flamegraphs by hiding framework internals.">

    <!-- Success State -->
    <div v-if="state === 'success'" class="alert alert-success d-flex align-items-center justify-content-between mb-4 py-2 small">
      <div>
        <i class="bi bi-check-circle me-2"></i>
        <strong>{{ lastResult!.affectedStacktraces }}</strong> stacktraces collapsed
        <span v-if="lastResult!.mergedStacktraces > 0"> ({{ lastResult!.mergedStacktraces }} merged)</span>.
      </div>
    ...
    <div>
      ...
      <div class="row align-items-start mb-3">
        <div class="col">
          <label class="form-label mb-1 small text-uppercase fw-semibold text-muted">Class Name Patterns</label>
        ...
        <div class="col">
          <label class="form-label mb-1 small text-uppercase fw-semibold text-muted">Synthetic Frame Label</label>
      ...
      <div v-if="state !== 'confirming'" class="d-flex align-items-center gap-2 mb-4">
        <button class="btn btn-sm btn-outline-primary" :disabled="!canPreview" @click="handlePreview">
          <span v-if="state === 'previewing'" class="spinner-border spinner-border-sm me-1"></span>
          <i v-else class="bi bi-eye me-1"></i>
          Preview
        </button>

        <button v-if="state === 'preview-ok'" class="btn btn-sm btn-primary" @click="state = 'confirming'">
      ...
      <!-- Confirmation Bar -->
      <div v-if="state === 'confirming'" class="alert alert-danger d-flex align-items-center justify-content-between mb-4">
        <div>
          <i class="bi bi-exclamation-diamond-fill me-1"></i>
          Collapse <strong>{{ previewResult!.affectedStacktraces }}</strong> stacktraces? This is permanent and cannot be undone.
        </div>
        <div class="d-flex gap-2 flex-shrink-0">
          <button class="btn btn-sm btn-outline-secondary" @click="state = 'preview-ok'">Cancel</button>
      ...
      <div v-if="state === 'error'" class="alert alert-danger d-flex align-items-start mb-4">
        ...
        <div>
          {{ errorMessage }}
          <button class="btn btn-sm btn-outline-secondary ms-2" @click="resetToIdle">Try Again</button>
        </div>
      </div>

      <!-- Preview Results -->
      <div v-if="previewResult && previewResult.samples.length > 0 && (state === 'preview-ok' || state === 'confirming')">
...
const canPreview = computed(() => {
  return nonEmptyPatterns.value.length > 0 && label.value.trim().length > 0 && state.value !== 'previewing'
})
