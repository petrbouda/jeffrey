...
<template>
  <PageHeader icon="bi-pencil-square" title="Rename Frames" description="Rename packages and classes in frame data to anonymize proprietary code before sharing graphs.">
    ...
    <div>
      ...
      <div class="row align-items-center mb-3">
        ...
        <div class="col">
          <label class="form-label mb-1 small text-uppercase fw-semibold text-muted">Replace with</label>
          <div class="input-group search-container">
      ...
      <div v-if="state === 'confirming'" class="alert alert-danger d-flex align-items-center justify-content-between mb-4">
        <div>
          ...
          Rename <strong>{{ previewResult!.affectedFrames }}</strong> frames? This is permanent and cannot be undone. To revert, you will need to regenerate the whole profile.
        </div>
        <div class="d-flex gap-2 flex-shrink-0">
          <button class="btn btn-sm btn-outline-secondary" @click="state = 'preview-ok'">Cancel</button>
          <button class="btn btn-sm btn-danger" @click="handleApply">
            Confirm Rename
          </button>
        </div>
      </div>
