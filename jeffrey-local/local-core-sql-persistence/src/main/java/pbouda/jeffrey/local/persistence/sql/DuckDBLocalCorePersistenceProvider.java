...
package pbouda.jeffrey.local.persistence.sql;

import pbouda.jeffrey.local.persistence.repository.*;

import pbouda.jeffrey.local.persistence.*;
...
public class DuckDBLocalCorePersistenceProvider implements LocalCorePersistenceProvider {
    ...
    public DuckDBLocalCorePersistenceProvider() {
        this.databaseProvider = new DuckDBPlatformDatabaseManager();
    }

    public DuckDBLocalCorePersistenceProvider(String migrationsLocation) {
    ...
    @Override
    public LocalCoreRepositories localCoreRepositories() {
        return new JdbcLocalCoreRepositories(new DatabaseClientProvider(dataSource), clock);
    }
