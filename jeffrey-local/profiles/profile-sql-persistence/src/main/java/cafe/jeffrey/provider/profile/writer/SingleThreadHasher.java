...
public class SingleThreadHasher {
    ...
    /**
     * Hash stacktrace from an array of frame hashes.
     * Note: profile scoping is handled at database level (per-profile databases)
     */
    public long hashStackTrace(long[] frameHashes) {
        ...
        int totalSize = frameHashes.length * 8;
    ...
    public long hashThread(EventThread thread) {
        ...
        // Calculate size: name + osId (8 bytes) + javaId (8 bytes)
        int totalSize = 4 + nameBytes.length + 8 + 8;
        byte[] bytes = new byte[totalSize];
