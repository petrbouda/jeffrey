...
package cafe.jeffrey.profile.guardian.guard.alloc;
...
public class ExceptionAllocGuard extends TraversableGuard {
    ...
    protected String summary() {
        ...
        return "The ratio between total allocations (" + result.totalValue() + ") and " +
                "allocations caused by exception creation (" + result.observedValue() + ") " +
    ...
    protected String explanation() {
        return """
                Exception objects and their stack traces consume memory. Each exception creation allocates
                the exception object itself plus a StackTraceElement array captured by fillInStackTrace().
                Frequent exception creation generates significant garbage, especially with deep call stacks.
                """;
    }
