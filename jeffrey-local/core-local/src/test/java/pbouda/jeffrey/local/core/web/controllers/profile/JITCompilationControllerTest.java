...
class JITCompilationControllerTest {
    ...
    @Mock
    JITCompilationManager jitManager;

    @Test
    void listsCompilations() {
        when(resolver.resolve("p-1")).thenReturn(profileManager);
        when(profileManager.jitCompilationManager()).thenReturn(jitManager);
        when(jitManager.compilations(20)).thenReturn(List.of());

        MockMvcTester mvc = mockMvcTesterFor(new JITCompilationController(resolver));

        assertThat(mvc.get().uri("/api/internal/profiles/p-1/compilation/compilations"))
                .hasStatusOk()
                .bodyJson()
