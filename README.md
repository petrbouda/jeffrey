...
| **[Microscope MCP](https://www.jeffrey-analyst.cafe/docs/microscope-mcp)**                      | Analyse your profiles from an interactive Claude Code, Codex or Gemini CLI session in your own repository — list recordings, query their tables, and pull flamegraph, trace and heap-dump exports, so the profile and the source code sit in front of the same reader. One `/plugin install microscope@jeffrey`, `codex plugin marketplace add petrbouda/jeffrey`, or `gemini extensions install --path`. |
...
| **[Jeffrey Heartbeat](https://www.jeffrey-analyst.cafe/docs/agent/heartbeat-library)**          | A tiny library (plus a Spring Boot starter) your application depends on. It writes a heartbeat file to the shared volume, so the Hub can track liveness and tell a clean shutdown from a crash. No Java agent, no network, no gRPC. |
| **[Jeffrey Events](https://central.sonatype.com/artifact/cafe.jeffrey-analyst/jeffrey-events)**  | A lightweight custom-JFR-event library (HTTP, gRPC, database, connection-pool) that powers the tech dashboards. |
...
- 🔌 **Microscope MCP (Claude Code, Codex & Gemini CLI plugin):** https://www.jeffrey-analyst.cafe/docs/microscope-mcp
