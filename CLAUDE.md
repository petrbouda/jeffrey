...
**shared** (`shared/`):
- `common` — Shared utilities and models. `SupportedFile` (`model/repository/`) is the **single registry of file types**: how a file is recognised on disk, whether it is a chunk of a session's recording, whether it is transient, whether the hub wrote it, whether it is a crash signal, whether Microscope parses it as a profile's recording, its event source and its statistics bucket. Nothing else carries a per-type switch or list; `UNKNOWN` is the fallback for a name no type recognises and is a plain downloadable file
