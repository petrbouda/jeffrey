...
- `microscope-core-sql-persistence` — DuckDB persistence for microscope core (recordings, profiles, hub connections). Workspaces and projects are not stored locally. **A recording is a list of files, not one file**: `Recording.recordingFiles()` are the files a profile is parsed from in reading order — one for an upload, one per JFR chunk for a session downloaded from a hub, ordered by name with the chunk extension stripped so compression does not reorder them — and `additionalFiles()` is everything beside them (heap dumps, logs, perf counters). Every parser entry point (`RecordingEventParser.start`, `ProfileInitializer.initialize`, `AutoAnalysisDataProvider.generate`, `ProjectRecordingStorage.findRecordingFiles`) takes that list; nothing joins the chunks into one file, because the JFR parser splits a recording into chunks to parse it anyway
...
- `core-hub` — Main Spring Boot app (HubApplication), gRPC service implementations, scheduler/jobs, JFR event replay. **The hub only serves files**: `FileDownloadService.DownloadFile` streams any finished, non-transient file of a session as it lies on disk — a JFR chunk (raw or `.jfr.lz4`), a heap dump, a log, an unclassified file — and there is no merge RPC. Microscope keeps the chunks it downloads as the files of one recording, in `Recording.recordingFiles()` order — nothing joins them, on either side: the hub once merged them and `RecordingDisassembler` split the result straight back into chunks for the parser. What the hub still keys on the chunk trait: LZ4 compression, age/quota trimming of an active session, replay, the environment parse, and the newest chunk inheriting a running session's status. Compression writes a hidden partial file and moves it into place atomically, so the `.jfr.lz4` that appears under a chunk's id is complete from the first moment it is listed; a chunk is dated by the timestamp in its name in either form, because dating the compressed form by the file system would put the oldest chunk last. The session listing does not follow links and `streamFile` refuses a path that resolves outside the session directory, because the producer side of the shared volume writes there
...
**shared** (`shared/`):
- `common` — Shared utilities and models. `SupportedFile` (`model/repository/`) is the **single registry of file types**: how a file is recognised on disk, whether it is a chunk of a session's recording, whether it is transient, whether the hub wrote it, whether it is a crash signal, whether Microscope parses it as a profile's recording, its event source and its statistics bucket. Nothing else carries a per-type switch or list; `UNKNOWN` is the fallback for a name no type recognises and is a plain downloadable file. `RecordingChunks` beside it is the one reading of "the session's JFR" off a file listing — finished chunks, one per id with the hub-compressed form preferred (a chunk keeps its id across compression and both forms lie side by side for a moment), oldest first — used by the hub's `RecordingSession.finishedChunks()` and by Microscope's session download alike
...
- gRPC proto files: `workspace_service.proto`, `project_service.proto`, `instance_service.proto`, `file_download_service.proto`, `repository_service.proto`, `profiler_settings_service.proto`, `event_streaming_service.proto`
- gRPC clients: `HubClients` record (in `shared/hub-client/`) containing `DiscoveryClient`, `RepositoryClient`, `FileDownloadClient`, `ProfilerClient`, `InstancesClient`, `ProjectsClient`, `EventStreamingClient`
...
| External MCP server (`core-microscope/.../mcp/`, `profiles/mcp-server`) + `jeffrey-claude-plugin/` | `docs/microscope-mcp/` — overview, enabling the server, Claude Code plugin, Codex plugin, Gemini extension, tool reference, skills, analysis agents, recipes, other clients |
