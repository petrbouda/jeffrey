...
### Database Schema
- Microscope Core migrations: `jeffrey-microscope/microscope-core-sql-persistence/src/main/resources/db/migration/microscope/core/` — `V001__init.sql` (table schema) + `V002__guardian_guards_seed.sql` (built-in Guardian guard seed data)
...
- **Migration policy**: Keep table schema (`CREATE TABLE`) in `V001__init.sql` and edit it in place for schema changes. Seed data may live in a separate, purpose-named migration (e.g. `V002__guardian_guards_seed.sql`) to keep schema and data concerns separated. The database is recreated from scratch on each startup, so editing these in development is safe.
