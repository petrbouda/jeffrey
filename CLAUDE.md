...
- **AI Integration**: the external MCP server (`POST /api/internal/mcp`); Spring AI survives only for the `@Tool`/`@ToolParam` annotations the MCP layer reflects over
...
- **MCP**: JSON-RPC 2.0 over Streamable HTTP — `POST /api/internal/mcp`, controller in `jeffrey-microscope/core-microscope/.../mcp/`, protocol layer in `jeffrey-microscope/profiles/mcp-server/`
