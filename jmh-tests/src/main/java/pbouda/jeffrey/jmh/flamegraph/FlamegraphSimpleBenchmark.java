...
import pbouda.jeffrey.jmh.flamegraph.mapper.OptimizedSimpleFlamegraphRecordRowMapper;
...
/**
 * Benchmark for the simple flamegraph query - aggregates all events for each stacktrace
 * across all threads. Returns one row per stacktrace with aggregated samples and weight.
 * No thread info or weight_entity.
 */
...
public class FlamegraphSimpleBenchmark {
    ...
    private static final Supplier<List<FlamegraphRecord>> OPTIMIZED_INVOCATION = () -> {
        ...
        OptimizedSimpleFlamegraphRecordRowMapper rowMapper = new OptimizedSimpleFlamegraphRecordRowMapper(
                Type.EXECUTION_SAMPLE, FramesCache.load(ds));
        return jdbcTemplate.query(sql, QUERY_PARAMS, rowMapper);
    };
     ...
     * <pre>
     * java -cp jmh-tests/target/benchmarks.jar pbouda.jeffrey.jmh.flamegraph.FlamegraphSimpleBenchmark
     * </pre>
     */
    public static void main(String[] args) throws RunnerException {
        Options options = new OptionsBuilder()
                .include(FlamegraphSimpleBenchmark.class.getSimpleName())
                .build();
