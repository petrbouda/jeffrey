...
jeffrey.local.guardian.reflection-warning-threshold=0.08
jeffrey.local.guardian.reflection-info-threshold=0.04

# Latency-tail guards are millisecond-based, not fractions.
jeffrey.local.guardian.safepoint-outlier-warning-millis=150
jeffrey.local.guardian.vthread-pinned-outlier-warning-millis=25

# Group-level minima: raise to require more data before a group runs.
jeffrey.local.guardian.min-samples-execution=5000`;
</script>
...
<template>
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <p>Every threshold and minimum-sample gate is bound to a property under <code>jeffrey.local.guardian.*</code>. All defaults ship with Jeffrey — you only set a property when you want to override one.</p>

      <DocsCodeBlock :code="propertiesExample" language="properties" />

      <p>Each rule in the frame-tree groups carries a pair:</p>
      <ul>
        <li><code>jeffrey.local.guardian.&lt;name&gt;-info-threshold</code> — ratio above which severity becomes INFO.</li>
        <li><code>jeffrey.local.guardian.&lt;name&gt;-warning-threshold</code> — ratio above which severity becomes WARNING.</li>
      </ul>
      ...
      <ul>
        <li><code>jeffrey.local.guardian.min-samples-execution</code> — default <code>1000</code></li>
        <li><code>jeffrey.local.guardian.min-samples-allocation</code> — default <code>1000</code></li>
        <li><code>jeffrey.local.guardian.min-samples-wall-clock</code> — default <code>1000</code></li>
        <li><code>jeffrey.local.guardian.min-samples-blocking</code> — default <code>100</code></li>
      </ul>

      <p>Latency-tail guards use millisecond thresholds, not ratios:</p>
      <ul>
        <li><code>jeffrey.local.guardian.safepoint-outlier-warning-millis</code> / <code>-info-millis</code></li>
        <li><code>jeffrey.local.guardian.vthread-pinned-outlier-warning-millis</code> / <code>-info-millis</code></li>
      </ul>

      <DocsCallout type="tip">
        The full, authoritative list of every property is the record <code>GuardianProperties.java</code> in <code>jeffrey-local/profiles/profile-guardian</code>. Each field's <code>@DefaultValue</code> annotation is the shipped default; each field name <code>camelCase</code> maps to the kebab-case property suffix shown above.
      </DocsCallout>
      ...
      <DocsNavFooter
        :prev="{ title: 'Profiles', path: '/docs/local/profiles' }"
      />
    </div>
  </article>
</template>
