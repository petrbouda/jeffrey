...
<template>
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <p>Drives the <a href="https://code.claude.com/docs" target="_blank" rel="noopener">Claude Code CLI</a> in headless mode, authenticating with your existing <strong>Claude Pro/Max subscription</strong> instead of an API key &mdash; so AI analysis incurs no per-token API charges. The <code>claude</code> CLI must be installed and logged in on the host running Jeffrey. Tool-driven analysis is served through an in-process MCP server that the CLI connects to over localhost, restricted to Jeffrey's read-only analysis tools (no shell or filesystem access). Available in the <strong>Microscope</strong> deployment for JFR and heap dump analysis.</p>
      <p>One setting widens that: <code>jeffrey.microscope.ai.source-access.enabled</code> lets an analysis <em>read</em> the checkout of the <router-link to="/docs/intellij-plugin">IntelliJ window</router-link> a profile is linked to, so it can point at the loop rather than stopping at a method name. It is <strong>off by default</strong> &mdash; it is the only setting that sends your source to the configured provider. With it on, the run gets that directory as its working directory and the reading tools only: no writing, no shell, and no other directory. A profile with no linked window is unaffected. Both assistants get it &mdash; a hot frame and a retained object both name a class, and the class&rsquo;s own code is what says what it does &mdash; and the system prompt tells the model the directory is the code that was profiled, with the rule that it opens a file before naming it. See <router-link to="/docs/microscope-mcp/enabling">Enabling the Server</router-link>.</p>
