...
selfProfile:
  enabled: true   # jeffrey-hub self-profiles via the JIB entrypoint`;
...
spec:
  ...
  ports:
    ...
    - port: 9090
      ...
      name: grpc
  selector:
    {{- include "jeffrey-hub.selectorLabels" . | nindent 4 }}`;
  ...
  <article class="docs-article">
    ...
    <div class="docs-content">
      <p>
        ...
        repo. Three charts under <code>helm/</code> coordinate a Jeffrey Hub, two
      ...
      <DocsCallout type="info">
        <strong>One chart per concern.</strong> <code>jeffrey-hub</code> owns the
        ...
        carry no PV/PVC/Ingress — they only bind to <code>jeffrey-hub</code>'s claim
        and stay cluster-internal.
      </DocsCallout>

      <h2 id="configure-server">Configuring Jeffrey Hub</h2>
      <p>
        From
...
spring.profiles.include=trace-file-log"
      ...
      <p>
        ...
        <code>http://jeffrey-hub.jeffrey-testapp.svc.cluster.local:8080</code> directly
        from the laptop. For other clusters, expose Jeffrey Hub via the chart's
      ...
      <p>
        ...
        extra flags to <code>jeffrey-hub</code> so it binds the PVC to a static
        hostPath PV instead:
      </p>
      ...
      <p>
        ...
        <code>jeffrey-hub</code> last (so its PVC outlives the consumers):
      </p>
...
helm uninstall jeffrey-hub         --namespace jeffrey-testapp"
      />
