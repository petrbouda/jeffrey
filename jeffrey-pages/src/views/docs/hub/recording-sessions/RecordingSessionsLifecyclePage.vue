...
<template>
  <article class="docs-article">
      ...
      <div class="docs-content">
        ...
        <p>Zero bytes alone is not enough to call a session failed, because the sizes are read straight off the hub's repository directory and a filesystem can misreport them: an object-storage FUSE mount, or an NFS mount with attribute caching, serves each file's size as of when the file was created, so a session that is recording perfectly well reports nothing but zeros for as long as that cache holds. Jeffrey therefore only reads a zero as a crash where a crash explains it — when the session produced <strong>no files at all</strong>, or when it died within a minute of starting, too early to have written anything.</p>

        <p>A session that holds files which all measure zero after running longer than that is shown as its own session, badged <strong>sizes unavailable</strong>, rather than collapsed into a failed band. Its files stay listed and stay downloadable — the download reads their real content, whatever the directory says about their size. Seeing that badge on a session you know is recording means the hub's mount is serving stale file metadata; compare <code>stat -c '%s' &lt;file&gt;</code> with <code>wc -c &lt; &lt;file&gt;</code> inside the hub container, and check the mount's attribute-cache settings (gcsfuse <code>metadata-cache:ttl-secs</code> / <code>stat-cache-ttl</code>, FUSE <code>attr_timeout</code>, NFS <code>actimeo</code>).</p>
