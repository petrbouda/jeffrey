  ...
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <DocsCallout type="info" title="Reading is read-only">
        Every analysis tool hands out data and nothing more &mdash; it cannot modify, rename or delete a profile, so data cleanup and frame renaming stay in the Jeffrey UI. The exceptions are the <code>recordings_</code> and <code>hubs_</code> families, which create profiles rather than changing them &mdash; from a local file, and from a recording still on a connected hub &mdash; <code>heap_prepare</code>, which builds a heap dump&rsquo;s index and reports, <code>operations_cancel</code>, which requests cancellation of background work, and the <code>ide_</code> family, which can link an IDE window to a profile and open a file in your editor. The two that reach outside this server, <code>hubs_</code> and <code>ide_</code>, can each be switched off on their own.
      </DocsCallout>
      ...
      <DocsCallout type="warning" title="Know what it exposes">
        The endpoint is on by default and has <strong>no authentication at all</strong> &mdash; there is no token to require, here or anywhere else in Jeffrey. Anyone who can reach the address can read every profile in that installation, ask it to open a recording file from that machine, and have it pull a recording down from a connected hub. Jeffrey binds every interface by default, so on a shared network &ldquo;anyone who can reach the address&rdquo; is wider than it sounds until you bind it to loopback. It does refuse a request whose <code>Host</code> is outside <code>jeffrey.microscope.mcp.allowed-hosts</code>, and one carrying a foreign <code>Origin</code>, which closes the browser path but nothing else. <router-link to="/docs/microscope-mcp/enabling">Enabling the Server</router-link> covers how to expose it safely behind a tunnel or proxy, how to narrow the tool surface with <code>families</code> and <code>preset</code>, how to turn hub access and the IDE bridge off on their own, and how to switch the whole thing off.
      </DocsCallout>
      ...
      <p><strong>Resources</strong> are the parts of a profile a client can attach rather than call for: the paginated catalogue at <code>jeffrey://profiles</code>, server build and capability information at <code>jeffrey://server</code>, runtime health at <code>jeffrey://diagnostics</code>, and templates for a profile&rsquo;s summary, its evidence snapshot and a flamegraph export. A tool result scrolls away; a resource a client has attached stays in view. Reading one runs the tool that would have answered the same question, so the two never disagree.</p>
