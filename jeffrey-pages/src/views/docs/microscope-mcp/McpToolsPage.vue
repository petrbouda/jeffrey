...
<template>
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>Pulls that session in and returns a <code>recordingId</code> for <code>recordings_analyzeRecording</code>. Alone, the whole session &mdash; every recording file it holds, fetched separately and kept separate, with its heap dumps and logs alongside. With <code>startTime</code>/<code>endTime</code> (UTC epoch milliseconds, either alone), only the chunks covering that window, every chunk whose span touches it, so the recording always covers the window with some slack at either end; with <code>fileIds</code> (comma-separated <code>file_id</code> values from <code>hubs_files</code>), the named files &mdash; whose chunks must be next to each other, because the recording reports one span across the files it holds and a skipped chunk leaves no hole to see; artifacts beside the run are free to name. A part of a session reports <code>windowStart</code>/<code>windowEnd</code>, the span its chunks cover, and is a recording of its own, never answered from a whole-session copy already here. Only JFR chunks are recordings on a hub &mdash; a pprof or OTLP file lying in a session is neither downloadable here as a recording nor fetchable with <code>hubs_fetchFile</code> as an artifact. A transfer that outlasts the call comes back with a status saying so and an <code>operationId</code>; <code>operations_status</code> is where to follow it</td>
          </tr>
          <tr>
            <td><code>hubs_files</code></td>
            <td><code>sessionRef</code></td>
            <td>Every file the session holds &mdash; the JFR chunks and, beside them, the application logs, the <code>gc.jvm-log</code>, the crash file, the perf-counters file, a heap dump &mdash; with its <code>file_id</code>, type, status and size, a <code>local</code> column: the absolute path of an artifact already on this machine (fetched, or brought along by <code>hubs_download</code>), or <code>recording:</code>/<code>profile:</code> for a chunk already pulled in as part of a local recording &mdash; and a <code>fetch</code> column saying how the row is reached: <code>fetch</code> takes its <code>file_id</code>, <code>hubs_download</code> is a recording chunk, and <code>no</code> is a type Jeffrey does not classify, which a hub will not serve one at a time. Every artifact is fetchable, including one of a session that is still recording: only the newest recording chunk is held open, and a log is worth grepping while it is being written. When the session is analysed, the note names the profile and its zero point, the instant an uptime in a GC log counts from</td>
          </tr>
          <tr>
            <td><code>hubs_fetchFile</code></td>
            <td><code>sessionRef</code>, <code>fileId</code></td>
            <td>Pulls one artifact down without the recording and returns the <strong>absolute path</strong> it now has on the machine Jeffrey runs on &mdash; beside the profile (<code>profiles/&lt;id&gt;/artifacts/</code>) when the session is analysed, under <code>artifacts/&lt;hub&gt;/&lt;project&gt;/&lt;session&gt;/</code> otherwise. A file already at its path comes back as it is &mdash; including one fetched before the session was analysed, which is moved beside the profile rather than transferred again. A transfer that outlasts the call carries an <code>operationId</code>; one that failed or was cancelled is started again by calling the tool again, with no retry flag. Recording chunks are refused in favour of <code>hubs_download</code></td>
          </tr>
        </tbody>
      </table>
      ...
      <p><strong>No OQL <em>assistant</em>.</strong> The OQL language itself is here &mdash; <code>heap_oql</code> runs it against the object graph, and answers with a link that opens the same query in the UI&rsquo;s OQL console. There is no tool that drafts a query from a question in English: over MCP the model writes its own, with the <router-link to="/docs/microscope-mcp/skills#heap-sql"><code>heap-sql</code> skill</router-link> as its reference.</p>
