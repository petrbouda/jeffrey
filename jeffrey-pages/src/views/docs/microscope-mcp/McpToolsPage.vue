...
#  -> { "recordingId": "019f885e-...", "chunkFiles": 2, "otherFiles": 2,
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>Every file the session holds &mdash; the JFR chunks and, beside them, the application logs, the <code>gc.jvm-log</code>, the crash file, the perf-counters file, a heap dump, a file Jeffrey does not classify &mdash; with its <code>file_id</code>, type, status and size, a <code>local</code> column: the absolute path of a file already on this machine (fetched, or brought along by <code>hubs_download</code>), or <code>recording:</code>/<code>profile:</code> for a chunk of a recording an earlier <code>hubs_download</code> joined into one file &mdash; and a <code>fetch</code> column saying how the row is reached: <code>fetch</code> takes its <code>file_id</code>, <code>hubs_download</code> is a recording chunk best taken with the whole session (<code>hubs_fetchFile</code> takes it alone too, as it lies on the hub), <code>when finished</code> is still being written, and <code>no</code> is a transient file the hub never serves. When the session is analysed, the note names the profile and its zero point, the instant an uptime in a GC log counts from</td>
          </tr>
          <tr>
            <td><code>hubs_fetchFile</code></td>
            <td><code>sessionRef</code>, <code>fileId</code></td>
            <td>Pulls one file down without the whole session &mdash; a log, a crash file, a heap dump, an unclassified file, or a single JFR chunk as it lies on the hub (<code>.jfr</code> or <code>.jfr.lz4</code>) &mdash; and returns the <strong>absolute path</strong> it now has on the machine Jeffrey runs on &mdash; beside the profile (<code>profiles/&lt;id&gt;/files/</code>) when the session is analysed, under <code>files/&lt;hub&gt;/&lt;project&gt;/&lt;session&gt;/</code> otherwise. A file already at its path comes back as it is &mdash; including one fetched before the session was analysed, which is moved beside the profile rather than transferred again. A transfer that outlasts the call carries an <code>operationId</code>; one that failed or was cancelled is started again by calling the tool again, with no retry flag. Only a transient file is refused</td>
          </tr>
        </tbody>
      </table>
