      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>Every file the session holds &mdash; the JFR chunks and, beside them, the application logs, the <code>gc.jvm-log</code>, the crash file, the perf-counters file, a heap dump &mdash; with its <code>file_id</code>, type, status and size, and a <code>local</code> column: the absolute path of an artifact already on this machine (fetched, or brought along by <code>hubs_download</code>), or <code>recording:</code>/<code>profile:</code> for a merged recording. When the session is analysed, the note names the profile and its zero point, the instant an uptime in a GC log counts from</td>
          </tr>
          <tr>
            <td><code>hubs_fetchFile</code></td>
            <td><code>sessionRef</code>, <code>fileId</code></td>
            <td>Pulls one artifact down without the recording and returns the <strong>absolute path</strong> it now has on the machine Jeffrey runs on &mdash; beside the profile (<code>profiles/&lt;id&gt;/artifacts/</code>) when the session is analysed, under <code>artifacts/&lt;hub&gt;/&lt;project&gt;/&lt;session&gt;/</code> otherwise. A file already at its path comes back as it is; a transfer that outlasts the call carries an <code>operationId</code>. Recording chunks are refused in favour of <code>hubs_download</code></td>
          </tr>
        </tbody>
      </table>
