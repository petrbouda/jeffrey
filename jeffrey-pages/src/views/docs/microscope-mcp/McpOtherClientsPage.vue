  ...
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <DocsCallout type="info" title="Every tool says whether it writes">
        Each spec in <code>tools/list</code> carries MCP <code>annotations</code>: <code>readOnlyHint</code>, <code>destructiveHint</code>, <code>idempotentHint</code> and <code>openWorldHint</code>. Almost everything Jeffrey exposes only reads a profile, and declares it. Tools that change state include: <code>recordings_analyzeFile</code> and <code>recordings_analyzeRecording</code>, which create a profile, <code>heap_prepare</code>, which builds a cache, <code>hubs_download</code>, which pulls a recording off another machine and creates one here, <code>operations_cancel</code> and <code>hubs_activityCancel</code>, which request cancellation of background work, and <code>ide_link</code> and <code>ide_open</code>, which act on the editor running beside Jeffrey. Every tool answers for itself, not for its family: <code>recordings_list</code>, <code>recordings_status</code> and <code>heap_status</code> only read, and say so. <code>destructiveHint</code> is false throughout &mdash; nothing here deletes a profile, a recording or a dump &mdash; and <code>openWorldHint</code> marks the <code>hubs_</code> and <code>ide_</code> families, and <code>operations_cancel</code>, which can cancel a remote Hub transfer. A client that gates approval on those hints does not need a hand-written deny-list.
      </DocsCallout>
