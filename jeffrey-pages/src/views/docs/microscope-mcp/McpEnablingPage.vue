...
<template>
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <p>Three things limit the blast radius even so. Every analysis tool is read-only &mdash; the nine that are not create a profile or a cache, delete a recording and its profile, start or stop background work, or act on the editor beside Jeffrey, and none of them rewrites a profile's data &mdash; and the SQL tools refuse a second statement after a semicolon rather than running it. The SQL engine itself is sandboxed: a profile database is opened with DuckDB's external file access and extension autoloading turned off, so a query is confined to that profile's tables and cannot read a file from the host or fetch anything over the network, however it is spelled. And the server has no shell: it answers questions about profiles, and does not run anything.</p>
