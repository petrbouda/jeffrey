  ...
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <DocsCallout type="warning" title="Enforced in Claude Code, instructed elsewhere">
        The Claude Code subagent is denied file tools and, tool by tool, the writers &mdash; the <code>recordings_</code> and <code>hubs_</code> families whole, <code>operations_cancel</code>, and <code>ide_link</code> and <code>ide_open</code> &mdash; in its own definition, so the first two rules hold whatever the model decides. The one writer it keeps is <code>heap_prepare</code>, because what that writes is a cache the heap questions it is given cannot be answered without. Codex has no per-agent tool deny-list: its copy is sandboxed read-only against your files, and the rest is instruction. Gemini has no deny-list either, only an allow-list its wildcards widen. To make it a wall in either, keep those families away from the whole session &mdash; <code>disabled_tools</code> on the <router-link to="/docs/microscope-mcp/codex">Codex</router-link> page, <code>excludeTools</code> on the <router-link to="/docs/microscope-mcp/gemini">Gemini CLI</router-link> one.
      </DocsCallout>
