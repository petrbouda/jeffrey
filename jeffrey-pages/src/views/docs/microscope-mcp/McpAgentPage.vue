...
<template>
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <p><code>profile-analyst</code> never calls <code>heap_prepare</code>, including when a shared skill recommends preparing a missing report. It reports the missing evidence to the caller; preparation belongs to the caller or <code>heap-triage</code>. Claude Code enforces that restriction through both supported tool prefixes. The Codex and Gemini analyst copies state it in their instructions.</p>
      ...
      <p>In <router-link to="/docs/microscope-mcp/gemini">Gemini CLI</router-link> two of them are files to copy as well &mdash; <code>gemini/agents/profile-analyst.md</code> and <code>gemini/agents/heap-triage.md</code> to <code>~/.gemini/agents/</code>, or <code>.gemini/agents/</code> for one repository. Gemini reads an extension's <code>agents/</code>, but validates that frontmatter strictly and rejects the keys Claude Code's agents carry, so the copies are written in its own dialect. There is no <code>profile-lead</code> for it: a Gemini subagent may not dispatch another subagent, and dispatch is all the lead does, so lead the investigation from the main conversation there.</p>
      ...
      <DocsCallout type="warning" title="Enforced in Claude Code, instructed elsewhere">
        The Claude Code subagent is denied file tools and, tool by tool, the writers &mdash; the <code>hubs_</code> family whole, every <code>recordings_</code> tool but <code>recordings_status</code>, <code>operations_cancel</code>, and <code>ide_link</code> and <code>ide_open</code> &mdash; in its own definition, so the first two rules hold whatever the model decides. It is denied <code>heap_prepare</code> too; the cache that tool builds belongs to <code>heap-triage</code>, the one agent that keeps it, because the heap questions it is given cannot be answered without one. Codex has no per-agent tool deny-list: its copy is sandboxed read-only against your files, and the rest is instruction. Gemini has no deny-list either, only an allow-list its wildcards widen. To make it a wall in either, keep those families away from the whole session &mdash; <code>disabled_tools</code> on the <router-link to="/docs/microscope-mcp/codex">Codex</router-link> page, <code>excludeTools</code> on the <router-link to="/docs/microscope-mcp/gemini">Gemini CLI</router-link> one.
      </DocsCallout>
      ...
      <p>What it holds is as deliberate as what it does: the orientation tools, <code>compare_list</code> and <code>compare_quality</code>, and the two specialists &mdash; and no export tool of its own, so it cannot drift into reading a flamegraph in the middle of coordinating. Like the other two it has no file tools, makes no recommendations and writes nothing. It writes to the <router-link to="/docs/microscope-mcp/skills#report"><code>report</code></router-link> skill&rsquo;s shape, and its model is inherited from the session rather than pinned.</p>
