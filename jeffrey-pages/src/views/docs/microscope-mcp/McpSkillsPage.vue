...
<template>
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <p>It also carries the two subsystems that have <strong>no family of their own</strong>. Garbage collection and JIT compilation are not offered by <code>flamegraph_list</code> and appear nowhere in a tool list, so without the skill a model asked why the pauses are long has no route at all; with it, the route is <code>jfr_</code> over <code>jdk.GarbageCollection</code>, <code>jdk.GCHeapSummary</code> and <code>jdk.GCPhasePause</code>, or over <code>jdk.Compilation</code>, <code>jdk.Deoptimization</code> and <code>jdk.CodeCacheStatistics</code>. Three things in that section are not inferable from the data: pause time is <code>sumOfPauses</code> and never the event&rsquo;s <code>duration</code> (which spans concurrent phases for ZGC, Shenandoah and G1); an empty <code>jdk.Compilation</code> result means nothing compiled <em>slowly</em>, not that nothing compiled; and the cause of GC is allocation, so a pause budget that shows GC matters ends in the allocation flamegraph, which is the only thing that names the code producing the garbage.</p>
      ...
      <p>It carries the GC and JIT queries too, because those are the questions with no purpose-built tool behind them: collections grouped by collector and cause and ranked by <code>sumOfPauses</code>; the <code>jdk.GCHeapSummary</code> pivot that turns two rows per <code>gcId</code> (<code>Before GC</code> / <code>After GC</code>) into reclaimed bytes and a live-set trend; the slowest compilations; and deoptimisations grouped by method <em>and</em> reason, which is what turns &ldquo;the JIT is busy&rdquo; into a method name and an <code>unstable_if</code> to go and read.</p>
