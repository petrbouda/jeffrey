...
<template>
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <p>Promoted spans are ordinary spans everywhere it matters — they count in the trace's span total, appear in the operation's span breakdown ("Socket read" ranks against your own operations), and subtract from their parent's self time — but they are marked apart: drawn solid in their wait category's colour rather than a span-kind pastel, and their detail names the event they came from. The event's own payload (host and port, file path, monitor class, parked class) is right there in the inline detail. Two toolbar toggles govern them along the reader's question: <strong>Blocking ops</strong> for the lock, park, sleep and stall rows, and <strong>I/O</strong> for the file and socket rows — so hiding the lock noise never hides the socket read that explains the trace. Switching both off reads the recorded span structure alone; a toggle whose family recorded nothing stays visible but disabled. The legend's category buttons narrow finer, one category at a time.</p>
