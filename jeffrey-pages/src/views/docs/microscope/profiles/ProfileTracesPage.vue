...
<template>
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <p><strong>Every throw carries its stack.</strong> Opening a throw in the span detail shows where it was thrown, and hovering its cross on the rail previews the top of it without opening anything. Both are <em>folded</em>: a server stack is around forty frames of which a handful are yours, so runs of library frames &mdash; the runtime, Spring, Tomcat, reflection &mdash; collapse into one bar each, leaving the throwing frame, your own frames, and the thread root. Nothing is discarded; a bar opens in place, and <code>Fold libraries</code> turns the whole thing off. A throw from inside a library has no frames of yours at all, and there the fold stands aside and shows the stack in full rather than collapsing it to nothing.</p>
      ...
      <ul>
        ...
        <li><strong>Traces Timeline</strong> — the slowest trace of this type and the trace count, bucketed over time. Aggregated by the database over <em>every</em> trace of the type, not over the capped fetch the other tabs read: for an operation called a third of a million times that fetch is the recording's first few seconds, and bucketing it drew a spike at the origin and an hour of flat zero. Slices with no trace in them come back as zeroes rather than being left out, so a stretch where the operation went quiet drops to the floor instead of being drawn as a gentle slope between the slices either side of it. The buckets span the whole recording, so an operation's shape can be read against the profile's clock and against another operation's.</li>
