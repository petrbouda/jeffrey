...
<template>
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <p>The busiest peers as a gallery of sparkline tiles — every peer's shape is visible before you pick one, so an outlier announces itself without a click. Selecting a tile plots that peer's full timeline underneath, marked with a <em>Single peer</em> badge so it is never confused with the aggregate Total chart.</p>
      <p>The same <strong>Throughput</strong> / <strong>Invocations</strong> tiles that drive the Total tab drive this gallery, and here the choice does more than relabel: the server ranks peers by the selected metric <em>before</em> it keeps the top twelve, so the two views show genuinely different peers. A cache or broker hammered by tiny reads ranks nowhere by volume and is simply absent from the Throughput gallery — switch to Invocations and it appears. The selected peer's own chart switches without a round-trip, since both metrics travel in one payload.</p>
