      ...
      <div class="workflow-steps">
        <div class="workflow-step">
          ...
          <div class="step-content">
            ...
            <p>Drop one or more files onto the dashed <strong>Drop recordings to analyze</strong> zone at the top of the page, or click <strong>Browse files…</strong>. The zone carries one lane per format Jeffrey can parse — <strong>JFR</strong>, <strong>pprof</strong>, <strong>OpenTelemetry</strong> and <strong>Heap dump</strong> — each listing its own extensions, so only <code>.jfr</code>, <code>.jfr.lz4</code>, <code>.hprof</code>, <code>.hprof.gz</code>, <code>.otlp</code>, <code>.pprof</code>, and <code>.pb.gz</code> are accepted; anything else is rejected inline with an error message and the valid files keep uploading.</p>
            <p>Uploads start <strong>automatically</strong> the moment files are dropped or selected — there is no &ldquo;Upload all&rdquo; button. While files are moving the lanes collapse to a colour legend and each file gets its own progress row inside the same panel, carrying the dot of the format it was recognised as; a completed row disappears about 5 seconds after that individual file finishes, so adding new files mid-batch never wipes earlier ones. Failed rows stay visible with a dismiss button. The active <strong>group</strong> chip on the row below the drop zone is the target group for these uploads — switch it before dropping to land in a different group.</p>
          </div>
        </div>
        <div class="workflow-step">
          ...
          <div class="step-content">
            ...
            <p>From a project's <strong>Instances</strong> view, open a session and click <strong>Download</strong>. The session's files are streamed to local storage, the JFR chunks assembled into one recording with the heap dumps and logs stored beside it, and the recording shows up tagged with its origin.</p>
