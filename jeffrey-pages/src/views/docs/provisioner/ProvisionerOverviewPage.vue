...
const headings = [
  { id: 'what-is-provisioner', text: 'What is Jeffrey Provisioner?', level: 2 },
...
provisioner init --base-config /path/to/config.conf
...
provisioner init --base-config /app/jeffrey.conf
...
<template>
  <article class="docs-article">
      ...
      <div class="docs-content">
        <p>Jeffrey Provisioner is a command-line tool that <strong>configures JVM processes</strong> for profiling, especially in containerized environments.</p>

        <h2 id="what-is-provisioner">What is Jeffrey Provisioner?</h2>
        <p>Jeffrey Provisioner reads a HOCON configuration file and generates output files containing JVM flags. These files are then used by your target JVM process.</p>
        ...
        <div class="how-it-works">
          ...
          <div class="flow-step">
            ...
            <div class="flow-content">
              <strong>Jeffrey Provisioner</strong>
        ...
        <p>The provisioner performs these tasks:</p>
        <ul>
        ...
        <p>Use Jeffrey Provisioner when you want to:</p>
        <ul>
        ...
        <p>A <strong>recording session</strong> is created each time the provisioner runs - typically when your application is deployed or restarted. Each session gets a unique identifier and its own directory.</p>

        <p>Each application <strong>instance</strong> (identified by a name derived from <code>project.instance-name</code>, the <code>HOSTNAME</code> environment variable, or a generated UUID) creates its own directory within the project. Sessions are nested inside instances — each restart creates a new session within the same instance directory.</p>

        <DocsCallout type="info">
          <strong>New deployment = New session:</strong> Every time your container starts or application restarts, Jeffrey Provisioner creates a fresh session within the instance directory. The same instance can accumulate multiple sessions over time, making it easy to track the history of a specific application instance.
        </DocsCallout>
        ...
        <p>For a container image built with JIB, you install nothing: the
          ...
          with <code>provisionerSource</code> (<code>native</code> or <code>jar</code>) and name the
          Jeffrey release with <code>payloadVersion</code>.</p>
        ...
        <DocsLinkCard
          ...
          icon="bi bi-gear"
          title="Configuration"
          description="Learn about HOCON configuration files, available options, features (heap dumps, perf counters), and profiler backends."
        />

        <h2 id="integration">Integration with Jeffrey</h2>
        <p>Sessions created by Jeffrey Provisioner appear in Jeffrey's <strong>Repository</strong> feature within Live and Remote workspaces.</p>
        ...
        <div class="workflow-steps">
          <div class="workflow-step">
            ...
            <div class="step-content">
              <strong>Provisioner Initializes Session</strong>
              <p>Application starts with Jeffrey Provisioner, creating a new session directory</p>
            </div>
          </div>
          <div class="workflow-step">
