...
└── .session-info.json           # Session metadata</code></pre>
        ...
        <p>Async-Profiler writes CPU, allocation, and lock samples to <code>profile-*.jfr</code> files. Its <code>jfrsync</code> setting captures JVM and Jeffrey instrumentation events in those recordings, including <code>jeffrey.AppInformation</code> identity metadata. The Hub serves the dumped files to Microscope one at a time, and the chunks it downloads stay as they were written — the several files one recording is made of, never joined into a single one.</p>
