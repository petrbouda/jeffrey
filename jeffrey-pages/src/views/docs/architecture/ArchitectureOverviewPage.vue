...
<template>
  <article class="docs-article">
      ...
      <div class="docs-content">
        ...
        <p>The machinery behind that timeline is shared, not copied per feature: a run is tracked by a registry that keeps at most one run per key and remembers the current or most recent one, while each stage records its own duration and exactly one stage at a time reports a live clock. A pipeline supplies only its stage ids and two policy choices — whether runs are capped by a concurrency ceiling, and whether a finished run is evicted after a while. Heap-dump initialization is uncapped and keeps its last run indefinitely, because the work is local CPU and IO and the settings page displays that run; a pipeline whose work is expensive or rate-limited caps its runs instead, and lets a finished one expire.</p>
        ...
        <p>What a run is keyed by is the feature's own business. The heap dump runs once per profile, so its key is the profile; a feature that fans out over several units of work files one run per unit and lets them drain through the shared ceiling a few at a time. The aggregate over those runs — what a page renders, and any rule about how many may be in flight — belongs to the feature, not to the pipeline.</p>
