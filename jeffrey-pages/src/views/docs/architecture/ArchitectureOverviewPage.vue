...
<template>
  <article class="docs-article">
      ...
      <div class="docs-content">
        ...
        <p>The machinery behind that timeline is shared, not copied per feature: one pipeline runs in the background under a registry that keeps at most one run per key and remembers the current or most recent one, while each stage records its own duration and exactly one stage at a time reports a live clock. A pipeline supplies only its stage ids and two policy choices — whether runs are capped by a concurrency ceiling, and whether a finished run is evicted after a while. Heap-dump initialization is uncapped and keeps its last run indefinitely, because the work is local CPU and IO and the settings page displays that run; an Advisor run holds a model call open for minutes, so it is capped and expires once its findings are stored.</p>
        <p>Live progress deliberately lives only in memory. A run interrupted by a restart is gone, which is truthful — the work died with the process. What survives is the terminal outcome: when a run finishes or fails, its stage timings are written to a <code>pipeline_runs</code> row in the profile database, keyed by pipeline and by what the run targeted, so a page reopened later still shows what happened.</p>
