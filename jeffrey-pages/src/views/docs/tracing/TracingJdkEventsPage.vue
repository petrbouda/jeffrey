...
<template>
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <p><strong>It names itself.</strong> A blocking event promotes to a fixed label — every <code>jdk.SocketRead</code> is "Socket read". A traced method promotes to <em>itself</em>, read from the event's own <code>method</code> field and shortened to <code>Class#method</code> for the row (the qualified name stays in the span's payload). The <code>#</code> is JFR's own separator and it is kept deliberately: it says "a method" in a span name the way <code>{service}/{method}</code> says "an RPC". It has to be that field: JEP 520 roots a MethodTrace stack trace at the <strong>caller</strong>, so the leaf frame names the method that made the call, not the one being traced.</p>
