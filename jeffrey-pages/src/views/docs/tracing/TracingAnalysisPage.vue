...
<template>
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <p>The span detail's <strong>Flamegraph</strong> action renders what ran inside the span, in either <strong>Inclusive</strong> or <strong>Self</strong> mode. Inclusive is the span's whole window plus the windows of every descendant that ran on another thread, so a phase that fans its work out to worker threads shows the workers' frames and not only its own thread waiting for them; Self subtracts the windows of same-thread children and merges what is left — the answer to "the parent is slow but its children are not, so what is it doing?". A child forked onto another thread is not subtracted in Self mode: the parent's thread was busy with its own work the whole time it ran.</p>
      ...
      <ul>
        ...
        <li><strong>Span flamegraph</strong> — with a per-span flamegraph open, the export button in the graph's toolbar replaces the trace's own and renders that graph: the same call tree the flamegraph AI export produces elsewhere, cut to the span's window in whichever mode (Inclusive or Self) is on screen. Its header says so &mdash; <code>scope: span</code>, with the number of windows and threads it covers &mdash; so an assistant does not read one span's hot path as the application's. On a weighted profile (allocation, blocking, method latency) every line of the call tree carries the subtree's weight next to its sample count, and the tree is pruned and ordered by that weight, so one sampled allocation of a large array is not lost among a hundred small ones.</li>
      </ul>
