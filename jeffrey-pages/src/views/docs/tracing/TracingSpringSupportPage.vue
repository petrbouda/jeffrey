...
<template>
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <p>That is the whole integration. The starter brings the <code>ThreadLocal</code> span storage, so it works on any JVM from Java&nbsp;21 up. On Java&nbsp;25, add <code>jeffrey-tracing-scoped-value</code> next to it to keep the span in a <code>ScopedValue</code> instead — it takes over automatically. The auto-configuration registers the filter first in the chain, names spans by the matched Spring MVC handler pattern, and binds <code>jeffrey.tracing.*</code>. It backs off entirely if you define your own filter, naming strategy or settings. Four auto-configurations decide independently what applies:</p>
      ...
      <p><code>jeffrey-tracing-spring</code> carries the same beans with no Spring Boot dependency and no auto-configuration — nothing happens until you ask. Unlike the starter it brings no span storage: add <code>jeffrey-tracing-scoped-value</code> (Java&nbsp;25+) or <code>jeffrey-tracing-thread-local</code> (Java&nbsp;21+) next to it.</p>
