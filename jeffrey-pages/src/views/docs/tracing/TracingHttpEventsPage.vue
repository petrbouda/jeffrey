...
<template>
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <p>Both extend <code>AbstractHttpExchangeEvent</code> (which extends <code>AbstractTracedEvent</code>) and carry: <code>method</code>, <code>uri</code>, <code>statusCode</code>, <code>remoteHost</code>, <code>remotePort</code>, <code>mediaType</code>, <code>queryParams</code> (JSON), <code>pathParams</code> (JSON), <code>requestLength</code> and <code>responseLength</code>. Both also inherit <code>attributes</code> from every traced event; on the <em>server</em> event it is what the customizers below fill, and on the client event nothing fills it.</p>
      ...
      <DocsCallout type="warning">
        <strong>Three rules, all of them about what the index will accept.</strong> Values must be <strong>flat scalars</strong> — text, a number or a boolean. Hand a customizer a map or a list and it is recorded as that object's <em>text</em> (<code>{a=1, b=2}</code>), which is searchable only as that exact string and is never what you wanted; the built-in header capture avoids the question by recording the first value of a header sent more than once, rather than a list. Values must be <strong>low-cardinality</strong> for the same reason span names are: past a couple of hundred distinct values a key stops being a browsable facet and becomes search-only, and every distinct value enters the recording's constant pool — so capture what you <em>group by</em>, never an identifier unique to each request. And <strong>never capture a credential</strong>: <code>Authorization</code> and <code>Cookie</code> do not belong in a file that gets uploaded, shared and kept.
      </DocsCallout>
