  ...
  <article class="docs-article">
    ...
    <div class="docs-content">
      ...
      <ul class="usecase-list">
        ...
        <li><i class="bi bi-check2-circle"></i> <strong>A recording panel in the editor</strong> — open a recording and its tab says whether Microscope has analysed it, offers the button that does, and lays the views out as tiles: flame graph, allocations, GC, threads, traces and the rest, each with a line on what the page answers. A ready recording adds its window, sample count, event types and sample-loss share; a ready heap dump adds retained size, instances, classes and GC roots, with tiles of its own — leak suspects, dominator tree, histogram, GC roots. Those are the only profile figures the plugin draws. Views the recording has no data for stay visible but dimmed, so an empty page is never a click away in disguise.</li>
        <li><i class="bi bi-check2-circle"></i> <strong>Hand it to a coding agent</strong> — one button starts Claude Code or Codex in a terminal with the profile already identified, so the agent reads it through Microscope's MCP server instead of trying to parse a JFR it cannot open. An agent that is not installed keeps its button, disabled. Switchable off under <em>Settings → Tools → Jeffrey Plugin</em>.</li>
