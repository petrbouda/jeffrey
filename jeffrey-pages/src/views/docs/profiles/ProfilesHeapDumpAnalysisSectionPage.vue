...
const headings = [
  ...
  { id: 'ai-analysis', text: 'AI-Powered Analysis', level: 2 }
];
  ...
  <article class="docs-article">
      ...
      <div class="docs-content">
        ...
        <div class="docs-feature-list">
          ...
          <DocsFeatureItem icon="bi bi-diagram-3" title="Dominator Tree">
            Hierarchical view of objects by retained memory size. Shows which objects dominate the heap by holding references to large object graphs. Start from the largest dominators and drill down to understand memory ownership.
          </DocsFeatureItem>
          <DocsFeatureItem icon="bi bi-search" title="Leak Suspects">
            Automated memory leak detection that identifies objects likely causing memory issues. Analyzes reference patterns and retained sizes to surface the most probable leak sources without manual investigation.
          </DocsFeatureItem>
        ...
        <div class="docs-feature-list">
          ...
          <DocsFeatureItem icon="bi bi-signpost-split" title="GC Root Path">
            Trace the reference chain from any object back to its GC root. Understand exactly why an object is being kept alive by following the path of references from the object to the root that prevents its garbage collection.
          </DocsFeatureItem>
        </div>
        ...
        <h2 id="ai-analysis">AI-Powered Analysis</h2>
        <p>When an AI provider is configured, additional analysis features become available:</p>

        <div class="docs-feature-list">
          <DocsFeatureItem icon="bi bi-robot" title="OQL AI Assistant">
            Generate OQL queries from natural language descriptions. Describe what you're looking for and the AI assistant creates the appropriate OQL query, making heap analysis accessible without memorizing OQL syntax.
          </DocsFeatureItem>
          <DocsFeatureItem icon="bi bi-stars" title="Heap Dump AI Analysis">
            AI-powered analysis that examines heap dump data to provide insights about memory usage patterns, potential issues, and optimization recommendations. Uses the heap dump's MCP server to query data directly.
          </DocsFeatureItem>
        </div>

        <DocsCallout type="info">
          <strong>AI provider required:</strong> AI features require configuring an AI provider (Claude or OpenAI) in application properties. See the Configuration section for setup instructions.
        </DocsCallout>
