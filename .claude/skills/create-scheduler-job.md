...
```java
case {JOB_TYPE_ENUM} ->
        new {JobName}JobDescriptor();  // or .of(params) if has parameters
```
...
```json
{
  ...
  "jobDefinitions": [
    ...
    "default-{job-name-kebab}"  // Add your job here
  ]
}
```
...
| ProjectRecordingStorageSynchronizerJob | RECORDING_STORAGE_SYNCHRONIZER | PROJECT | Syncs recordings between storage and database |
