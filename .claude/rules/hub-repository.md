...
- `shared/common` holds utilities and hub↔provisioner contract types only (`RemoteProject*`, `ConfigScope`/`ConfigType`/`ConfigSource`, `ScopedConfigLayout`, `RepositoryType`, `JeffreyLayout`, `ChunkWindow`). Never put a domain record there.
