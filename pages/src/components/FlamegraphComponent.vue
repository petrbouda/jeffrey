...
const props = defineProps(['profileId', 'flamegraphId', 'eventType', 'scrollableWrapperClass', 'timeRange']);
...
let profileId, flamegraphId, eventType, scrollableWrapperClass, timeRange;
...
onMounted(() => {
  ...
  } else if (eventType != null && timeRange != null) {
    request = FlamegraphService.generateEventTypeRange(profileId, eventType, timeRange);
  } else if (eventType != null) {
    request = FlamegraphService.generateEventTypeComplete(profileId, eventType);
  } else {
  ...
  MessageBus.on(MessageBus.FLAMEGRAPH_TIMESERIES_RANGE_CHANGED, (timeRange) => {
    FlamegraphService.generateEventTypeRange(profileId, eventType, timeRange)
        .then((data) => {
          flamegraph = new Flamegraph(data, 'flamegraphCanvas');
          flamegraph.drawRoot();
        });
