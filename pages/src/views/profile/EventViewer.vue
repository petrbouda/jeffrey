...
<template>
  ...
  <div class="card">
    ...
    <TreeTable :value="allEventTypes" :filters="filters" :filterMode="filterMode.value" v-model:expandedKeys="expandedKeys">
      <Column field="name" header="Name" :expander="true" filter-match-mode="contains" style="padding: 5px">
        <template #filter>
          <InputText v-model="filters['name']" type="text" class="p-column-filter" placeholder="Filter by Name"/>
        </template>

        <template #body="slotProps" style="padding: 5px">
            <span class="font-bold"  v-if="slotProps.node.data.code == null">{{ slotProps.node.data.name }}</span>
            <span class="text-primary" v-else>{{ slotProps.node.data.name }} - <span class="p-column-title text-red-400">{{ slotProps.node.data.count }}</span></span>
        </template>
      </Column>
      <Column field="code" header="Code" filter-match-mode="contains" style="padding: 5px">
        <template #filter>
      ...
      <Column headerStyle="width: 10rem" style="padding: 5px">
        <template  #body="slotProps">
          <div class="flex flex-wrap gap-2 flex-row-reverse" v-if="slotProps.node.data.code != null">
            <Button type="button" icon="pi pi-search text-xs" />
            <Button type="button" icon="pi pi-pencil text-xs" severity="success" />
          </div>
        </template>
      </Column>
    </TreeTable>
