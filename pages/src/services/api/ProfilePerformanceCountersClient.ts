...
export default abstract class ProfilePerformanceCountersClient {
    ...
    public static async exists(profileId: string): Promise<Boolean> {
        let baseUrl = GlobalVars.internalUrl + '/profiles/' + profileId + '/perfcounters/exists';
        return axios.get<Boolean>(baseUrl, HttpUtils.JSON_ACCEPT_HEADER)
            .then(HttpUtils.RETURN_DATA)
    }

    public static async get(profileId: string): Promise<PerformanceCounterEnhanced[]> {
        let baseUrl = GlobalVars.internalUrl + '/profiles/' + profileId + '/perfcounters';
