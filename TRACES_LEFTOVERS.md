...
| **Context is attributed to the span an event starts in** | An event is charged to the innermost span open on its thread when it began — now actually enforced by a per-event innermost pick in `SPAN_CONTEXT` (the window join alone matched every enclosing span, and nested spans double-counted; the comment claimed innermost before the SQL did it). A wait that straddles a child's start is still charged wholly to whichever span it started in rather than split between them — splitting would need the same interval arithmetic self time uses, per category | A straddling wait is seen to matter in practice |
| **Synthesized spans carry minted ids** | The derivation deliberately stopped inventing span ids — except for the blocking leaf spans it promotes out of `jdk.*` events, which can never carry a recorded id: the choice there is a minted id or no bar. Minted ids are hashed into `[1, 2^63-1]` (never 0, never negative), so they are recognisable and cannot read as "absent"; recorded spans still never get one | A promoted id collides with a recorded one in practice (the derivation anti-joins to survive it, silently dropping the promoted row) |
| **`ThreadState` not extended with `jdk.FileForce` / `jdk.VirtualThreadPinned`** | The promotion added both to `TraceContextCategory` (FILE_IO grew, VT_PINNED is new), but the thread-timeline's parallel enum is a bijection with its lanes and growing it means new lanes, colours and queries in `profile-threads` — a separate feature surface. This is exactly the drift the enum's javadoc warns about, recorded here instead of half-done | The threads timeline should show fsync and pinning lanes |
| **The "GC phases" waterfall toggle is gone** | It only expanded the `jdk.GCPhasePauseLevel1..4` breakdown of a pause the lane already draws — a GC-investigation drill-down, not latency information, and it redrew the same stopped world up to five times. The nested pauses stay recorded and `TracePause.nested` still marks them; only the entry point was removed | A GC investigation wants the phase bands back — the data is one toggle away |
| **`jdk.TLSHandshake` is not promoted** | Whether it is recorded as a duration event was not verified; promoting an instant event would draw zero-length "handshakes" | Its duration semantics are confirmed on a real recording |
| **Promoted payload is not attribute-indexed** | A promoted span's host/port/path/monitorClass live in `event_fields` (rendered in the inline detail) but `attributes` is NULL, so the attribute search cannot filter on them | Users want to search traces by I/O host or lock class |
...
- ~~Blocking time was a chip, not a bar~~ — the derivation now promotes blocking JDK events
  (`jdk.SocketRead/Write`, `jdk.FileRead/Write/Force`, `jdk.JavaMonitorEnter`, `jdk.JavaMonitorWait`,
  `jdk.ThreadPark`, `jdk.ThreadSleep`, `jdk.ZAllocationStall`, `jdk.VirtualThreadPinned`) into
  synthesized leaf spans under the innermost span open on their thread (`BlockingLeafSpans` is the
  single place the promoted set lives). Each carries its real recorded duration and payload, comes
  out of its parent's self time, counts in `span_count` and the operation breakdown, and renders in
  the waterfall in its context category's colour behind a "Blocking ops" toggle plus the per-category
  legend filters. The same categories leave `SPAN_CONTEXT` and the span drill-down (the bar replaces
  the chip and the row — counting both said the same blocked microsecond twice), and the why-slow
  summary rebuilds their slices from the synthesized spans so the panel and the bars are one source
  of truth. This started as a ByteBuddy-agent idea (instrument the `jdk.*` commits to call
  `Tracer.stamp`); analysis-time promotion won because these events are synchronous and thread-bound,
  so the offline join records exactly what the agent would have — with zero recording overhead and
  retroactively on every existing profile.
