...
class CreateSessionWorkspaceEventConsumerIntegrationTest {
    ...
    private static final HubJeffreyDirs JEFFREY_DIRS = new HubJeffreyDirs(Path.of("/tmp/jeffrey-test"));
    ...
    class InstanceStatusTransition {
        ...
        void sessionCreation_transitionsInstanceToActive(DataSource dataSource) throws SQLException {
            ...
            var provider = new DatabaseClientProvider(dataSource);
            var clock = new MutableClock(NOW);
            var realHubPlatformRepositories = new JdbcHubPlatformRepositories(provider, clock);
            ...
            var instanceRepo = realHubPlatformRepositories.newProjectInstanceRepository(PROJECT_ID);
            ...
            var consumer = new CreateSessionWorkspaceEventConsumer(
                    realHubPlatformRepositories, JEFFREY_DIRS, sessionFinisher);
            consumer.on(event, projectsManager);
