...
/**
 * Checks an async-profiler command.
 *
 * <p>The command is passed through to a JVM's argfile, so the checks here are about what would make
 * that file malformed or the value meaningless, not about whether the profiler will like it — the
 * hub does not know async-profiler's grammar and should not pretend to.</p>
 */
public class AsprofSettingsValidator implements ConfigValueValidator {
