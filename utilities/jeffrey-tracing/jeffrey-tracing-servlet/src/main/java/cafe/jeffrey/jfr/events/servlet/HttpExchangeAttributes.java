...
public final class HttpExchangeAttributes {
    ...
    /**
     * Longest value recorded before it is truncated and marked, the same limit {@code @Traced}
     * argument capture and MyBatis parameter capture apply. One oversized header must not bloat
     * every event in a recording.
     */
    ...
    public HttpExchangeAttributes put(String key, Object value) {
        ...
        if (value == null || AttributeValues.text(value, MAX_VALUE_LENGTH).isBlank()) {
            return this;
        }
        if (!keys.add(key)) {
            return this;
        ...
        AttributeValues.put(attributes, key, value, MAX_VALUE_LENGTH);
        return this;
    }
