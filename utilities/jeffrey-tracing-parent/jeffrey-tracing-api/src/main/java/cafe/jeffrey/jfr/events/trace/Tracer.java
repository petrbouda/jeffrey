 ...
 * <p>
 * The span currently in progress is bound on the calling thread for the duration of the body, so a
 * nested {@link #call} or {@link #run} discovers its parent without the caller threading anything
 * through. The binding is bounded by the lambda, which means it cannot outlive the span and never
 * needs to be cleared.
 * <p>
 ...
 *   <li><b>The current span does not follow work to another thread.</b>
...
public final class Tracer {
    ...
    private static final SpanContextStorage STORAGE = SpanContextStorages.load();
    ...
    public static <R, X extends Throwable> R callChecked(
            String name, SpanBody<? extends R, X> body) throws X {
     ...
     * to {@code Exception}.
     * <p>
     * A distinct name rather than a {@code call} overload: a result-bearing lambda matches
     * {@link Supplier} and {@link SpanBody} alike, and javac resolves the overload to the {@link Supplier}
     ...
     */
    public static <R, X extends Throwable> R callChecked(
            String name, SpanKind kind, SpanBody<? extends R, X> body) throws X {
     ...
     * and the binding is gone by the time a caller's {@code finally} block runs,
    ...
    public static <R, X extends Throwable> R inSpanOf(
            AbstractTracedEvent event, SpanBody<? extends R, X> body) throws X {
        ...
        return STORAGE.callWith(openSpanOf(event), body);
    }

    /**
     * The {@link Runnable} form of {@link #inSpanOf(AbstractTracedEvent, SpanBody)},
    ...
    public static <R, X extends Throwable> R reenter(
            SpanContext context, SpanBody<? extends R, X> body) throws X {
        ...
        if (!event.isEnabled()) {
            ...
            return STORAGE.callWith(context, body);
        }
    ...
    /**
     * The {@link Runnable} form of {@link #reenter(SpanContext, SpanBody)}, for a
    ...
    public static void stamp(AbstractTracedEvent event) {
        Objects.requireNonNull(event, "event must not be null");
        SpanContext current = STORAGE.current();
        if (current != null) {
            stampSelf(event, current.child());
        }
    }
     ...
     * bound — the way to keep a trace connected across an executor boundary, which the current span
     * does not cross on its own.
     * <p>
     * Pass {@code null} to start a fresh trace.
     */
    public static <R, X extends Throwable> R continueIn(
            SpanContext parent, String name, SpanKind kind, SpanBody<? extends R, X> body) throws X {
    ...
    /**
     * The form of {@link #continueIn(SpanContext, String, SpanKind, SpanBody)} with
     * kind {@link SpanKind#INTERNAL} — forked work is in-process work unless declared otherwise.
     */
    public static <R, X extends Throwable> R continueIn(
            SpanContext parent, String name, SpanBody<? extends R, X> body) throws X {
        return continueIn(parent, name, SpanKind.INTERNAL, body);
    }

    /**
     * The {@link Runnable} form of
     * {@link #continueIn(SpanContext, String, SpanKind, SpanBody)}, for a task with no
    ...
    public static Runnable fork(String name, SpanKind kind, Runnable body) {
        ...
        SpanContext parent = STORAGE.current();
    ...
    private static SpanContext newSpanContext() {
        return childOf(STORAGE.current());
    }
    ...
    private static <R, X extends Throwable> R record(
            ...
            SpanKind kind,
            SpanContext context,
            SpanBody<? extends R, X> body) throws X {
