...
5. **A trace does not cross a plain executor by itself.** The span in progress
   is bound to the thread that opened it. Either wrap the pool once
...
operation. The current span is bound to the thread for the duration of each
body — nothing is threaded through your call signatures.
...
The span in progress does not survive a plain executor hand-off. Three tools:
